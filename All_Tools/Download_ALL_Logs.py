...
for FTP_HOST,site_name in zip(List_of_IPS,Names_OF_SITES):
   ...
   #directory_structure = sftp.listdir_attr() # Obtain structure of the remote directory
        for Network_Type in Types_of_Network:
           ...
           # print(size_file_1[3])
            sftp_1.get('/tl3010/var/log/kpi_'+Network_Type+'.log','C:/Users/m.ali/All_Sites/'+site_name+'/kpi_'+Network_Type+date.today().strftime('%d-%m-%Y')+'.txt',callback=lambda x,y: progressbar_1(x,y))
            #################################
            #print("############### New Download ##########################")
